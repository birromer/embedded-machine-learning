...
The *VERBOSE* compilation variable was also added to increase the information in stdout during evaluation. Most notably, it will add specify what is being processed in the various steps of the methods, and during evaluation will print a labeled confusion matrix instead of plain numbers, such as bellow.

#+begin_example
Using extracted testing features:./DATA/features_testing.csv
SVM accuracy: 0.603333
Confusion matrix:
...
#+begin_src bash :tangle no :export result :results verbatim
sh ./eval.sh
#+end_src
