...
=[[file:setup.sh][setup.sh]]= and =[[file:eval.sh][eval.sh]]= are the tangled snippets presented in this file, with everything up to model training in the former, and the performance evaluation in the latter.
...
It outputs to the data folder the files =[[file:DATA/file_list_train.txt][file_list_train.txt]]= and =[[file:DATA/file_list_test.txt][file_list_test.txt]]=, with the file paths for training and testing. The testing one is the input for the evaluation binaries, if no other is provided.
...
The training script is located at =[[file:SVM/train_svm.py][SVM/train_svm.py]]= and the prediction at =[[file:SVM/prediction_svm.cpp][SVM/prediction_svm.cpp]]=.
